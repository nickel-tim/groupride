"""Checks liga/migrations/0001_init.sql against SQLite (D1 is SQLite).
Run:  python3 liga/test/schema_check.py
Checked: schema loads, foreign keys/CHECKs take effect, ranking queries use the
index (no scan of the whole table), deleting cascades."""
...
# Ranking sum (kilometres) in the period, members only
...
# Best value (top speed): maximum per account
...
# Explore: only new tiles in the period, via index
...
# Cascade: deleting a ride takes values, track, sharing with it
