...
with sync_playwright() as p:
    ...
    navs = pg.evaluate("[...document.querySelectorAll('nav button')].map(b => b.textContent.trim())")
    check('tab bar in English', navs == ['Ride', 'Rides', 'League', 'More'], navs)
    ...
    scan('Ride (ready)')
    pg.click('#btnGhostPick'); pg.wait_for_timeout(300); scan('Ghost picker'); pg.click('#pickClose')
    pg.click('#btnSim'); pg.click('[data-warp="20"]'); pg.wait_for_timeout(8000)
    for v, name in (('compass', 'Compass (simulation running)'), ('map', 'Map'), ('prof', 'Profile'), ('log', 'Events')):
        pg.click('[data-lv="%s"]' % v); pg.wait_for_timeout(700); scan(name)
    pg.click('[data-lv="compass"]'); pg.wait_for_timeout(300)
    ...
    pg.click('#btnStop'); pg.wait_for_timeout(1500)
    scan('Ride (ready) with ghost choice')
    pg.click('nav button[data-v="rides"]'); pg.wait_for_timeout(400); scan('Rides')
    pg.click('#rideList .ride'); pg.wait_for_timeout(300); scan('Ride details')
    check('ride details in English', 'view summary' in visible(pg).lower() and 'replay the group' in visible(pg).lower(), visible(pg)[:200])
    pg.click('#rideActs [data-act="sum"]'); pg.wait_for_selector('#sumBody .sumttl', timeout=30000); pg.wait_for_timeout(500)
    t = visible(pg).lower(); check('summary in English', 'front work' in t and 'distance' in t, t[:200]); scan('Summary'); pg.click('#sumClose')
    pg.click('#rideList .ride'); pg.click('#rideActs [data-act="replay"]'); pg.wait_for_timeout(2500); scan('Replay'); pg.keyboard.press('Escape')
    pg.click('nav button[data-v="rides"]')
    for sub in ('seg', 'rec'):
        pg.click('[data-sub="%s"]' % sub); pg.wait_for_timeout(400); scan('Rides / ' + sub)
    pg.click('[data-sub="seg"]'); pg.click('#segNew'); pg.wait_for_timeout(500); scan('Segment editor'); pg.click('#seCancel')
    pg.click('nav button[data-v="ride"]'); pg.click('#btnQr'); pg.wait_for_timeout(400); scan('QR overlay'); pg.keyboard.press('Escape')
    pg.click('nav button[data-v="more"]'); pg.wait_for_timeout(300); scan('More')
    ...
    for tab in ('stand', 'halle', 'strecken', 'mehr'):
        pg.click('[data-act="tab"][data-tab="%s"]' % tab); pg.wait_for_timeout(500); scan('League / ' + tab)
    pg.click('[data-act="tab"][data-tab="stand"]'); pg.click('[data-act="goals-edit"]'); pg.click('[data-act="goal-add"][data-scope="my"]'); pg.wait_for_timeout(200); scan('League goals editor')
    ...
    check('static text is German again', pg.evaluate("[...document.querySelectorAll('#v-more h2')].map(h => h.textContent)")[0] == 'Ich')
    navs = pg.evaluate("[...document.querySelectorAll('nav button')].map(b => b.textContent.trim())")
    check('tab bar German again', navs == ['Ausfahrt', 'Fahrten', 'Liga', 'Mehr'], navs)
